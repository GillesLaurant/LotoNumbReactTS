...
function Loto() {
    ...
    return (<section className="loto">
      <div className="loto-grids-container">
        
        <Grids handle={handleUserClick} nameCss={"userNums"} min={1} max={49} jackpot={jackpot.find((item) => item.name === "userNums")}/>
        
        <Grids handle={handleUserClick} nameCss={"userStars"} min={1} max={10} jackpot={jackpot.find((item) => item.name === "userStars")}/>
      </div>
