...
function EuroMillions() {
    ...
    const [jackpot, setJackpot] = useState([
        { name: "nums", table: returnArray(1, 5, "*") },
        { name: "stars", table: returnArray(1, 2, "*") },
        { name: "userNums", table: returnArray(1, 5, "*") },
        { name: "userStars", table: returnArray(1, 2, "*") },
    ]);
    // Grid user's click
    const handleUserClick = (arr, v) => {
        // Copy state
    ...
    const handleJackpot = () => {
        ...
        const tableStars = getJackpot(jackpot[1].table, 12, jackpot[3].table);
        setJackpot([
    ...
    return (<section className="euro">
      <div className="euro-grids-container">
        
        <Grids handle={handleUserClick} nameCss={"userNums"} min={1} max={50} jackpot={jackpot.find((item) => item.name === "userNums")}/>
        
        <Grids handle={handleUserClick} nameCss={"userStars"} min={1} max={12} jackpot={jackpot.find((item) => item.name === "userStars")}/>
      </div>
